...
Route::post('/character/upgrade', function (Request $request) {
    ...
    $trend = DB::table('training_runs')
        ...
        ->map(function ($r) {
            ...
            return [
                ...
                'date' => \Illuminate\Support\Carbon::parse($r->created_at)->format('m/d'),
            ];
        })
        ->all();
            ...
            'recent' => collect($endlessRecent->get($key, []))
                ...
                ->map(function ($r) {
                    ...
                    return [
                        ...
                        'accuracy' => $all > 0 ? round($r->typed_chars / $all * 100, 1) : 0.0,
                        'exp' => $r->exp_gained,
                        'date' => \Illuminate\Support\Carbon::parse($r->created_at)->format('m/d H:i'),
                    ];
                })
                ->all(),
        ];
