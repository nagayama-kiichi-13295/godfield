...
<div class="arena">
    <div class="bars">
        <div class="bar-block">
            <div class="bar-label">{{ $player->name }}（{{ $characterName }} Lv.{{ $player->level }}）</div>
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    let idx = 0, pos = 0, myHp = MY_MAX, cpuHp = CPU_MAX;
    let playing = false, finished = false, cpuTimer = null, cpuIdx = 0;
    ...
    async function finish(won) {
        ...
        try {
            const res = await fetch('/solo/result', {
                ...
                body: JSON.stringify({ won: won, difficulty: difficulty }),
            });
            const d = await res.json();
            overlaySub.textContent = d.leveled
                ? `EXP +${d.gain}　レベルアップ！ Lv.${d.level}`
                : `EXP +${d.gain}　（Lv.${d.level}　${d.exp}/${d.required}）`;
        } catch (e) {
    ...
    function scheduleCpu() {
        ...
        cpuTimer = setTimeout(() => {
            ...
            myHp -= CPU_POWER;
            updateHp();
            if (myHp <= 0) { finish(false); return; }
            scheduleCpu();
        }, wait);
    }

    function begin() {
        idx = 0; pos = 0; myHp = MY_MAX; cpuHp = CPU_MAX; cpuIdx = 0;
        finished = false;
        updateHp();
    ...
    startBtn.addEventListener('click', () => { startBtn.blur(); begin(); });
